...
const breakSession = new Timer(0.2);
let currentStatus = 1;
// debugger;
...
function breakTimeUpNoise() {
  var whaleNoise = new Audio('/sounds/humpback-whale.wav');
  var waveNoise = new Audio('/sounds/waves.wav');
  if (breakSession.remainingTime < 21000 && breakSession.remainingTime > 20000) {
    waveNoise.play();
  }
  if (breakSession.remainingTime < 16000 && breakSession.remainingTime > 15000) {
    waveNoise.play();
  }
  if (breakSession.remainingTime < 11000 && breakSession.remainingTime > 10000) {
    waveNoise.play();
  }
  if (breakSession.remainingTime < 6000 && breakSession.remainingTime > 5000) {
    waveNoise.play();
  }
  if (breakSession.remainingTime < 1000 && breakSession.remainingTime > 0) {
    whaleNoise.play();
  }
}
